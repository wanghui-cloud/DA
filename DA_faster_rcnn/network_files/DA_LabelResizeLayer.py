...
class ImageLabelResizeLayer(nn.Module):
    ...
    # 输入：# x：[bs, 2, H, W]   need_backprop：0、1
    def forward(self, x, need_backprop):

        # 不计算梯度
        feats = x.detach().cpu().numpy()
        lbs = need_backprop.detach().cpu().numpy()
        # 创建float32的空numpy，[bs,H,W,1]
        gt_blob = np.zeros((lbs.shape[0], feats.shape[2], feats.shape[3], 1),
                           dtype=np.float32 )
        for i in range(lbs.shape[0]):
            lb = np.array([lbs[i]])
            lbs_resize = cv2.resize( lb, (feats.shape[3], feats.shape[2]),
