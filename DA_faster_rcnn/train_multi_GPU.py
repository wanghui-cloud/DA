# --------------------------------------------------------
# 一、训练过程：
# 运行命令：python -m torch.distributed.launch --nproc_per_node=4 --use_env train_DA_multi_GPU.py
# pth模型保存位置：‘./save/model/DA/backbone名称_数据集名称/model_num.pth’
# 图片结果位置：‘./save/outputs/DA/backbone名称/’存放：
...
def main(args):
    ...
    # 用来保存coco_info的文件
    results_file = os.path.join(args.record_dir, args.backbone, "{}_results{}.txt"
...
if __name__ == "__main__":
    ...
    # pth模型保存地址
    parser.add_argument('--output-dir-weight', default='./save/model/multi_train', help='path where to save')
    # png、txt保存地址
    parser.add_argument('--record-dir', default='./save/outputs/multi_train', help='path where to save')
    # 基于上次的训练结果接着训练
    ...
    if not os.path.exists(os.path.join(args.output_dir_weight,
                                       args.data_path.split('/')[-1] + "_" + args.backbone)):
        os.makedirs(os.path.join(args.output_dir_weight,
                                 args.data_path.split('/')[-1] + "_" + args.backbone))
